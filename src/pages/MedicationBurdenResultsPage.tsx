...
import { ArrowLeft, AlertCircle, CheckCircle2, TrendingUp, Pill, BarChart3, Clock, Target, BookOpen, Loader2, Mail, Download  } from 'lucide-react';
...
export function MedicationBurdenResultsPage() {
  ...
  useEffect(() => {
    const loadReport = () => {
      try {
        ...
        if (storedAssessmentType !== 'Medication Burden') {
      ...
      try {
        ...
        fetch('https://luther.health/api/send-email-report', {
          ...
          body: JSON.stringify({
            ...
            assessmentType: 'Medication Burden',
            report: aiReport,
            reportId: Date.now(),
  ...
  const comparisonData = aiReport?.results.map(result => ({
    ...
    average: Math.min(Math.max(result.score - 10, 35), 50),
    optimal: Math.max(result.score - 30, 15)
  })) || [];
  ...
  const getOverallRating = (rating: string) => {
    const levelMap: Record<string, 'low' | 'moderate' | 'high' | 'severe'> = {
      'Low Burden': 'low',
      'Moderate Burden': 'moderate',
      'High Burden': 'high',
      'Severe Burden': 'severe'
    };
    ...
    return (
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Analyzing your medication profile...</p>
          </div>
        </div>
    );
  }
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          <Card className="mb-8">
            <CardHeader className="text-center">
              <div className="flex items-center justify-center space-x-2 mb-2">
                ...
                <CardTitle>Medication Assessment Complete</CardTitle>
              </div>
              <CardDescription>
            ...
            <div className="space-y-8">
              ...
              {results.map((result, index) => {
                const analysis = result.detailedAnalysis || {
                  ...
                  strengths: ['Good medication awareness'],
                  riskFactors: ['Professional review recommended'],
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            Burden Score: {result.score}/{result.maxScore} • Level: {Math.round((result.score / result.maxScore) * 100)}th percentile
                          </CardDescription>
                        </div>
                        <Badge variant={getScoreBadgeVariant(result.level)} className={getScoreColor(result.level)}>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    <span>Personalized Medication Safety Plan</span>
                  </CardTitle>
                  <CardDescription>
                  ...
                  <div className="space-y-6">
                    ...
                    {results.every(result => result.level === 'low') && (
                      <div className="text-center py-8">
                        ...
                        <h3 className="font-medium mb-2">Low Medication Burden!</h3>
                        <p className="text-muted-foreground">
                          Your assessment shows excellent medication management. Continue your current practices and regular pharmaceutical reviews.
                        </p>
                      </div>
                    )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Monitor medication burden and consider regular pharmaceutical reviews for ongoing optimization.</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
          ...
          {allTabsViewed && (
            <div className="flex justify-center">
              <Button onClick={() => window.location.hash = 'medication-burden-feedback'} size="lg" className="px-8">
                Next
              </Button>
            </div>
          )}
