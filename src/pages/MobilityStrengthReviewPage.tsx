    ...
    if (rating > 0 || selectedPositiveFeedback.length > 0 || selectedNegativeFeedback.length > 0) {
      ...
      if (rating >= 4) {
        const conclusions = [
          ...
          "I'm giving this a strong recommendation for anyone wanting to track their recovery progress.",
          "Would absolutely use this again and recommend it to others.",
          "This has genuinely helped me understand my baseline and recovery potential."
        ];
        review += conclusions[Math.floor(Math.random() * conclusions.length)];
      } else if (rating === 3) {
        const conclusions = [
          "It's worth trying if you're looking for mobility assessment tools.",
          "A decent option among the various baseline measurement resources available.",
