...
export function ResilienceIndexReviewPage() {
  ...
  const generateReview = () => {
    ...
    const createFeedbackSentence = (positive: string[], negative: string[]) => {
      ...
      if (positive.length > 0) {
        const positiveConnectors = [
          ...
          "The resilience analysis was notably",
    ...
    if (rating > 0 || selectedPositiveFeedback.length > 0 || selectedNegativeFeedback.length > 0) {
      ...
      if (rating >= 4) {
        const conclusions = [
          "Overall, I'd definitely recommend this to anyone looking to build mental resilience.",
          "I'm giving this a strong recommendation for anyone interested in understanding their mental toughness.",
          "Would absolutely use this again and recommend it to others focused on mental strength development.",
          "This has genuinely helped me understand my resilience capacity and growth opportunities."
        ];
        review += conclusions[Math.floor(Math.random() * conclusions.length)];
      } else if (rating === 3) {
        const conclusions = [
          "It's worth trying if you're looking for resilience assessment tools.",
          "A decent option among the various mental toughness evaluations available.",
          "Has some valuable elements that make it worth considering for resilience building.",
        ...
        const conclusions = [
          ...
          "With some improvements, this could become a much more valuable mental health resource.",
